/* =====================================================================
   kerne.js - faelles hjaelpefunktioner for sb1.4

   Alt bor i det globale objekt NK. Ingen moduler og ingen fetch:
   mappen skal ogsaa virke, naar index.html aabnes direkte fra
   harddisken (file://). Samme kerne som sb1.1 plus NK.bet (tal med
   betydende cifre uden tierpotens) og NK.laesTal (et tal, eleven har
   skrevet).
   ===================================================================== */
var NK = window.NK || {};
window.NK = NK;
...
(function () {
    ...
    /* Et tal med et antal betydende cifre og uden tierpotens:
       0,0500  0,120  25,0  0,303 */
    NK.bet = function (v, cifre) {
        cifre = cifre || 3;
        if (v === 0 || !isFinite(v)) return "0";
        var e = Math.floor(Math.log10(Math.abs(v)));
        if (parseFloat(Math.abs(v / Math.pow(10, e)).toFixed(cifre - 1)) >= 10) e += 1;
        return NK.tal(v, Math.max(0, cifre - 1 - e));
    };

    /* Et tal, eleven har skrevet: komma eller punktum, og gerne en
       tierpotens som 1,5·10^-2, 1,5*10-2, 1,5·10⁻² eller 1,5e-2.
       NaN, hvis det ikke er et tal. */
    NK.laesTal = function (s) {
        var HAEV = "⁰¹²³⁴⁵⁶⁷⁸⁹";
        s = String(s || "").trim().replace(/\s+/g, "").replace(/−/g, "-").replace(/,/g, ".").replace(/M$/i, "");
        s = s.replace(/[⁻⁺⁰¹²³⁴⁵⁶⁷⁸⁹]+$/, function (h) {
            return h.split("").map(function (c) { return c === "⁻" ? "-" : (c === "⁺" ? "" : String(HAEV.indexOf(c))); }).join("");
        });
        var m = s.match(/^([-+]?\d*\.?\d+)(?:(?:[·*x×]10\^?|e)([-+]?\d+))?$/i);
        if (!m) return NaN;
