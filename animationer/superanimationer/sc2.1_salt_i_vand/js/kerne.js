/* =====================================================================
   kerne.js - faelles hjaelpefunktioner for sc2.1
...
(function () {
    ...
    /* Et gramtal, som det ville staa i en tabel: store tal uden decimaler,
       smaa tal med lige saa mange, som der skal til, foer de kan ses.
       0,0002 g og 246 g skal begge kunne staa samme sted. */
    NK.gram = function (v) {
        if (!isFinite(v)) return "-";
        if (v === 0) return "0";
        var a = Math.abs(v);
        if (a >= 100) return NK.tal(v, 0);
        if (a >= 10) return NK.tal(v, 1);
        if (a >= 1) return NK.tal(v, 2);
        if (a >= 0.01) return NK.tal(v, 3);
        if (a >= 0.001) return NK.tal(v, 4);
        return NK.tal(v, 5);
    };
